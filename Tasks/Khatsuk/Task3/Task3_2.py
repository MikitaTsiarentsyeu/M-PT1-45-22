def string_split(base_string: 'исходная строка'):
    "возвращает список слов из переданной строки"
    base_string = base_string.lower()
    #оставляем только буквы и пробельные символы
    base_string = ''.join(filter(lambda s: s.isalpha() or s.isspace(), base_string))
...
while True :
    ...
    base_string = string_split(base_string)
    if not base_string:
    ...
    while True:
        ...
        if elements_qantity.isdigit():
            ...
            if 0 < elements_qantity <= len(result_list):
                ...
                print('нужна цифра от одного до количества слов!!!!!!')
